...
object Education {
  def main(args: Array[String]): Unit = {
    ...
    My_Utils.my_log("Getting education labeled dataset")
    val education_labeled_dataset = Dataset.get_estate_label_from_questionnaire(sparkSession, questionnaire_table_name)

    My_Utils.my_log("Geting all data with features including labeled and unlabeled set")
    val (dataset_labeled, dataset_unlabeled) = Dataset.get_dataset(sparkSession, education_labeled_dataset, imei_features_table, feature_dim, job_date)
