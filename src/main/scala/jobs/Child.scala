...
object Child {
  def main(args: Array[String]): Unit = {
    ...
    val xxx_table_name = "algo.yf_xxx_labels"
    val feature_dim: Int = 30000
    val model_save_dir = "/apps/recommend/models/wind/parent"
    ...
    My_Utils.my_log("Getting child labeled dataset")
    val child_labeled_dataset = Dataset.get_child_label_from_xxx_data(sparkSession, xxx_table_name)

    My_Utils.my_log("Geting all data with features including labeled and unlabeled set")
    val (dataset_labeled, dataset_unlabeled) = Dataset.get_dataset(sparkSession, child_labeled_dataset, imei_features_table, feature_dim, job_date)

    My_Utils.my_log("Building lr model and training")
    val model = Logistic_Regression.build_model(sparkSession, dataset_labeled, dataset_unlabeled, classes_num = 3, model_refined_flag = false, model_save_dir, job_date)
