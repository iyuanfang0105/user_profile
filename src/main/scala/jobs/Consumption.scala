...
object Consumption {
  def main(args: Array[String]): Unit = {
    ...
    My_Utils.my_log("Getting consumption labeled dataset")
    val consumption_labeled_dataset = Dataset.get_parent_label_from_questionnaire(sparkSession, questionnaire_table_name)

    My_Utils.my_log("Geting all data with features including labeled and unlabeled set")
    val (dataset_labeled, dataset_unlabeled) = Dataset.get_dataset(sparkSession, consumption_labeled_dataset, imei_features_table, feature_dim, job_date)
