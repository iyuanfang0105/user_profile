...
object Income {
  def main(args: Array[String]): Unit = {
    ...
    My_Utils.my_log("Getting income labeled dataset")
    val income_labeled_dataset = Dataset.get_income_label_from_questionnaire(sparkSession, questionnaire_table_name)

    My_Utils.my_log("Geting all data with features including labeled and unlabeled set")
    val (dataset_labeled, dataset_unlabeled) = Dataset.get_dataset(sparkSession, income_labeled_dataset, imei_features_table, feature_dim, job_date)
